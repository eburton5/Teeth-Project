function [rslt] = ComputeContinuousProcrustes4(GM,GN,options)
...
if strcmpi(FeatureMatchType,'TPS')
    [TextureCoords1,cPmap] = TPSDeformation(GM,GN,cPmap,FeatureType,[real(pushGM);imag(pushGM)],TextureCoords2,options);
elseif strcmpi(FeatureMatchType,'Laplacian')
    [TextureCoords1,cPmap] = LaplacianDeformation(GM,GN,cPmap,FeatureType,[real(pushGM);imag(pushGM)],TextureCoords2,options);
elseif strcmpi(FeatureMatchType,'BaryCentric')
    [TextureCoords1,cPmap] = BaryCentricDeformation(GM,GN,cPmap,FeatureType,TextureCoords1,TextureCoords2,options);
end
...
%%%%% try linear interpolation in the computation of cPdist
...
cPdist = MapToDist(GM.V(:,nBV),imagPts,1:length(nBV),GM.Aux.VertArea(nBV)); % align teeth then calc cPdist
