...
%% Mesh v12
%load('Lemuridae.mat'); G = Lemuridae{5}; % a mesh G will be generated
% load('Lemuridae_fixed_for_crack_deliv.mat'); G = Lemuridae{3};
load('aj05_ffc.mat');
...
%     rslt12 = G{1}.ComputeContinuousProcrustes(G{2},options);
    ...
    % VforDist2 = zeros(3,length(v12.V));
%     v12 = load('./samples/PNAS/v12.mat'); v12 = v12.G;
%     V23 = v12.V;
%     Vnew23 = V23./(sqrt(Area));
%     v12.V = Vnew23;
%     % want to keep only the coordinates cooresponding to those in the subsample
%     for x = 1:length(v12.V)
%         if x ~= sample{j}
%             v12.V(:,x) = Inf;
%         end
%     end
%     % now calculate distance matrix
%     Dist{j} = zeros(length(v12.V));
%     for l1 = 1:length(v12.V)
%         for l2 = 1:length(v12.V)
%             Dist{j}(l1,l2) = sqrt((v12.V(1,l1)-v12.V(1,l2))^2 + (v12.V(2,l1)-v12.V(2,l2))^2 + (v12.V(3,l1)-v12.V(3,l2))^2);
%         end
%     end
%     t{j} = Dist{j}(:);
%     t{j}(isinf(t{j})) = [];
...
save('aj05_ffc_1000_Rslt.mat','cPDist');
...
save('aj05_ffc_1000_CPDself.mat','CPDself');
